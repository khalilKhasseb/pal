<div class="mt-6 container mx-auto px-2 md:px-4">
    ...
    <x-slot name="breadcrumbs">
        @if($post->parent !== null)
        <li class="">
            <a href="{{ route('page',[$post->parent->slug]) }}" class="" aria-current="page">{{ $post->parent->title
                }}</a>
    ...
    <img alt="{{ $post->title }}" src="{{ $post->image('pages') }}"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2rem] rounded-tl-none shadow-md px-10 pb-6 ">
        <div class="flex items-center justify-between">
            <span class="font-light text-gray-600 dark:text-gray-100">{{ optional($post->published_at)->diffForHumans()
                ?? '' }}</span>
            <div>
