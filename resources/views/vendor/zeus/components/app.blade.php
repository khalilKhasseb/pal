            ...
            <div class="flex justify-between h-16">
                <div class="flex">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex sm:items-center">
                        ...
                        $menu =\LaraZeus\Sky\SkyPlugin::get()->getModel('Navigation')::fromHandle('primary');

                        @endphp

                        @if(!is_null($menu))
                        @foreach($menu->items as $item)
                        {!! \LaraZeus\Sky\Classes\RenderNavItem::render($item,'px-5 py-2 text-sm font-medium
                        text-gray-600 hover:text-blue-500 dark:text-gray-400') !!}

                        {{--
                        <x-zeus.nav-item :item='$item' :statePath='"path"' /> --}}
                        @endforeach
